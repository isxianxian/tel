<template>
    <div class="home h-100 bg-gray">
        <TopNav :back="false" :tit="navTit" v-show="isRouterAlive"></TopNav>
        <div class="main">
            <router-view
                v-if="isRouterAlive"
                @changeCur="changeCur"
    ...
    export default {
        ...
        methods: {
            ...
            loginPlan() {
                ...
                if (!code) {
                    let href =
                        'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxd12e5a279b145f0a&redirect_uri=https%3A%2F%2Fyuncoding.cn%2Findex.html&response_type=code&scope=snsapi_base&state=123#wechat_redirect'

                    // href = 'http://localhost:8080/index.html#/home?code=1'
            ...
            userLogin(code) {
                // let res = '996c66ed-a6fe-463b-b26d-0f2f41b7f1e2'
                // this.loginState(true)
                // localStorage.setItem('user-token', res)
                // let goPayPage = Number(localStorage.getItem('goPayPage'))
                // Boolean(goPayPage)
                ...
                return this.$api.login(code).then((res) => {
                    if (res) {
                        ...
                        let goPayPage = Number(localStorage.getItem('goPayPage'))
                        Boolean(goPayPage)
                            ? this.$router.push('/pay?back=true')
                            : (this.isRouterAlive = true)
